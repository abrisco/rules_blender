...
def _download_and_extract_7zip_binary(repository_ctx):
    ...
    zip_exe = repository_ctx.path(zip_data.bin_path)

    if zip_data.url.endswith(".exe"):
        repository_ctx.download(
            url = zip_data.url,
            output = zip_data.bin_path,
...
def _dmg_archive_impl(repository_ctx):
    ...
    z7_dir, z7_bin = _download_and_extract_7zip_binary(repository_ctx)
    ...
    out_dir = repository_ctx.path("dmg_out")

    repository_ctx.report_progress("Extracting dmg...")
    result = repository_ctx.execute([z7_bin, "x", dmg, "-o{}".format(out_dir), "-y"])
    ...
    # Navigate into the output directory which is implicit to the archive
    target_dir = out_dir.readdir()[0]

    # Check to see if any prefixes can be stripped
    if repository_ctx.attr.strip_prefix:
        target_dir = target_dir.get_child(repository_ctx.attr.strip_prefix)
    ...
    for item in target_dir.readdir():
        if item.basename in ["[]", "[HFS+ Private Data]", ".DS_Store", ".background.tif"]:
            continue
        repository_ctx.rename(item, repository_ctx.path(item.basename))
...
""",
    implementation = _dmg_archive_impl,
    attrs = _ATTRS,
)
